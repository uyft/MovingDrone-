...
parser.add_argument('--save-vis', dest='save_vis', action='store_true',
                    help='save density maps, heatmaps, overlays and count CSV for visualization')
parser.add_argument('--vis-output', dest='vis_output', default='visualization_data',
                    type=str, help='visualization output root directory')
parser.add_argument('--seqs', dest='sequences', nargs='+', type=int, default=None,
                    help='only test specified sequence IDs, e.g. --seqs 11 15')
parser.add_argument('--overlay-alpha', dest='overlay_alpha', default=0.45,
                    type=float, help='heatmap opacity in overlay images')
parser.add_argument('--heatmap-percentile', dest='heatmap_percentile', default=99.5,
                    type=float, help='percentile used to normalize heatmap previews')
parser.add_argument('--skip-point-save', dest='skip_point_save', action='store_true',
                    help='do not overwrite existing loc/trk/cyc point TXT files')


def scalar_float(value):
    """Convert a Python number, NumPy scalar or single-element tensor to float."""
...
def frame_attributes(seq_id, small, cloudy, sunny, crowd):
    """Return the video-level attributes already defined by the original code."""
    if seq_id in cloudy:
        weather = 'cloudy'
    elif seq_id in sunny:
        weather = 'sunny'
    else:
        weather = 'night'

    return {
        'scale_group': 'small' if seq_id in small else 'large',
        'weather': weather,
        'crowd_level': 'crowded' if seq_id in crowd else 'sparse',
    }


def density_to_heatmap(density_map, percentile=99.5):
    """
    Convert a non-negative density matrix to an OpenCV BGR heatmap.
    The raw density matrix is saved separately, so percentile normalization
    affects preview colors only and does not change numerical results.
    """
...
def save_visualization_frame(
    ...
    """Save raw density matrix, heatmap preview and full-resolution overlay."""
    sequence_dir = os.path.join(output_root, 'sequence_{:03d}'.format(seq_id))
    density_dir = os.path.join(sequence_dir, 'density_npy')
    heatmap_dir = os.path.join(sequence_dir, 'heatmap')
    overlay_dir = os.path.join(sequence_dir, 'overlay')

    os.makedirs(density_dir, exist_ok=True)
    os.makedirs(heatmap_dir, exist_ok=True)
    os.makedirs(overlay_dir, exist_ok=True)

    stem = 'img{:03d}{:03d}'.format(seq_id, frame_id)

    density_path = os.path.join(density_dir, stem + '_density.npy')
    heatmap_path = os.path.join(heatmap_dir, stem + '_heatmap.png')
    overlay_path = os.path.join(overlay_dir, stem + '_overlay.jpg')

    density = np.asarray(density_map, dtype=np.float32)
    np.save(density_path, density)

    heatmap = density_to_heatmap(density, percentile=percentile)
    cv2.imwrite(heatmap_path, heatmap)

    original = cv2.imread(image_path)
    if original is None:
        raise FileNotFoundError('Cannot read original image: {}'.format(image_path))
    ...
    return density_path, heatmap_path, overlay_path


def write_visualization_outputs(rows, output_root):
    """Write all-sequence and per-sequence CSV/JSON files for Spring Boot."""
    if not rows:
        return
    ...
    all_csv = os.path.join(output_root, 'counts_all.csv')
    with open(all_csv, 'w', newline='', encoding='utf-8-sig') as file:
    ...
    by_sequence = {}
    for row in rows:
        by_sequence.setdefault(int(row['sequence_id']), []).append(row)

    sequence_summaries = []

    for seq_id, seq_rows in sorted(by_sequence.items()):
        sequence_dir = os.path.join(
            output_root,
            'sequence_{:03d}'.format(seq_id),
        )
        os.makedirs(sequence_dir, exist_ok=True)

        seq_csv = os.path.join(sequence_dir, 'counts.csv')
        with open(seq_csv, 'w', newline='', encoding='utf-8-sig') as file:
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(seq_rows)

        errors = np.asarray(
            [float(row['error']) for row in seq_rows],
            dtype=np.float64,
        )
        ...
        sequence_summaries.append({
            'sequenceId': seq_id,
            'frameCount': len(seq_rows),
            'meanGroundTruthCount': float(np.mean([
                float(row['ground_truth_count']) for row in seq_rows
            ])),
            'meanPredictedCount': float(np.mean([
                float(row['predicted_count']) for row in seq_rows
            ])),
            'mae': float(np.mean(abs_errors)),
            'rmse': float(np.sqrt(np.mean(errors ** 2))),
            'meanError': float(np.mean(errors)),
            'scaleGroup': seq_rows[0]['scale_group'],
            'weather': seq_rows[0]['weather'],
            'crowdLevel': seq_rows[0]['crowd_level'],
        })

    summary_path = os.path.join(output_root, 'sequence_summary.json')
    with open(summary_path, 'w', encoding='utf-8') as file:
        json.dump(
            {
                'frameCount': len(rows),
                'sequenceCount': len(sequence_summaries),
    ...
    print('Visualization counts CSV:', all_csv)
    print('Visualization summary JSON:', summary_path)
...
def main():
    ...
    for img_path in glob.glob(os.path.join(test_path, '*.jpg')):
        seq_id = int(os.path.basename(img_path)[3:6])
        if args.sequences is None or seq_id in args.sequences:
            test_list.append(img_path)
    test_list.sort()

    if len(test_list) == 0:
        raise RuntimeError(
            'No test images found for sequences: {}'.format(args.sequences)
        )

    if args.sequences is not None:
        print('Selected sequences:', args.sequences)
        print('Selected test frames:', len(test_list))
    test_clip = []
    test_step = 0
...
def test(val_pair, model, save_path_prefix):
    ...
    if args.save_vis:
        ...
        print('Visualization output:', os.path.abspath(args.vis_output))
    ...
    for i, (imgs1, imgs2, imgs3, imgs4, gtnum, gt) in enumerate(test_loader):
        ...
        den_g11, den_g12, den_g13, loc_g11, loc_g12, loc_g13, loc_g14, reg_g11, reg_g12, reg_g13, reg_g14, trk_s14, trk_o14, trk_p14 = model(
            imgs1)
        den_g21, den_g22, den_g23, loc_g21, loc_g22, loc_g23, loc_g24, reg_g21, reg_g22, reg_g23, reg_g24, trk_s24, trk_o24, trk_p24 = model(
            imgs2)
        den_g31, den_g32, den_g33, loc_g31, loc_g32, loc_g33, loc_g34, reg_g31, reg_g32, reg_g33, reg_g34, trk_s34, trk_o34, trk_p34 = model(
            imgs3)
        den_g41, den_g42, den_g43, loc_g41, loc_g42, loc_g43, loc_g44, reg_g41, reg_g42, reg_g43, reg_g44, trk_s44, trk_o44, trk_p44 = model(
            imgs4)

        # calculate density result
        denout1 = max(0, den_g11[1].data.sum()) + max(0, den_g21[1].data.sum()) + max(0, den_g31[1].data.sum()) + max(0,
                                                                                                                      den_g41[
                                                                                                                          1].data.sum())
        denout2 = max(0, den_g12[1].data.sum()) + max(0, den_g22[1].data.sum()) + max(0, den_g32[1].data.sum()) + max(0,
                                                                                                                      den_g42[
                                                                                                                          1].data.sum())
        denout3 = max(0, den_g13[1].data.sum()) + max(0, den_g23[1].data.sum()) + max(0, den_g33[1].data.sum()) + max(0,
                                                                                                                      den_g43[
                                                                                                                          1].data.sum())
        ...
        if args.use_loc and not args.use_trk:
            loc1 = torch.exp(loc_g14[1][:, 0, :, :]) / (
                        torch.exp(loc_g14[1][:, 1, :, :]) + torch.exp(loc_g14[1][:, 0, :, :]))
            loc2 = torch.exp(loc_g24[1][:, 0, :, :]) / (
                        torch.exp(loc_g24[1][:, 1, :, :]) + torch.exp(loc_g24[1][:, 0, :, :]))
            loc3 = torch.exp(loc_g34[1][:, 0, :, :]) / (
                        torch.exp(loc_g34[1][:, 1, :, :]) + torch.exp(loc_g34[1][:, 0, :, :]))
            loc4 = torch.exp(loc_g44[1][:, 0, :, :]) / (
                        torch.exp(loc_g44[1][:, 1, :, :]) + torch.exp(loc_g44[1][:, 0, :, :]))
        ...
        # Save visualization data without changing the original point output.
        if args.save_vis:
            current_image_path = val_pair[i][1]
            image_name = os.path.basename(current_image_path)
            image_stem = os.path.splitext(image_name)[0]
            frame_id = int(image_stem[-3:])

            # The original model scales density values by args.factor=100.
            # Save a non-negative count-density map for visualization.
            density_map = np.maximum(
                out_data.astype(np.float32) / float(args.factor),
                0.0,
            )

            predicted_count = scalar_float(denout1 / args.factor)
            ground_truth_count = float(gtnum[0][1][0])
            ...
            density_path, heatmap_path, overlay_path = save_visualization_frame(
                image_path=current_image_path,
                density_map=density_map,
                output_root=args.vis_output,
                seq_id=seqID,
                frame_id=frame_id,
            ...
            visualization_rows.append({
                ...
                'image_path': os.path.abspath(current_image_path),
                'predicted_count': predicted_count,
                'ground_truth_count': ground_truth_count,
                'error': error,
                'absolute_error': abs(error),
                'density_sum_clipped': float(density_map.sum()),
                'scale_group': attributes['scale_group'],
                'weather': attributes['weather'],
                'crowd_level': attributes['crowd_level'],
        ...
        if (i % 5 == 0 or i == len(test_loader) - 1):
            print('img{i:d}*GT {gtnum:d} Pos {posnum:d} Neg {negnum:d} MAE1 {mae1:.1f} MAE2 {mae2:.1f} MAE3 {mae3:.1f}'
                  .format(i=i + 1, gtnum=allgtnum, posnum=allposnum, negnum=allnegnum, mae1=mae1 / (i + 1),
                          mae2=mae2 / (i + 1), mae3=mae3 / (i + 1)))

    if args.save_vis:
        write_visualization_outputs(
            visualization_rows,
            os.path.abspath(args.vis_output),
        )
    ...
    print('*Overall MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae) / len(test_loader),
                                                        mse=math.sqrt(scalar_float(mse) / len(test_loader))))

    print('*Small MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_small) / max(1., count_small),
                                                      mse=math.sqrt(scalar_float(mse_small) / max(1., count_small))))
    print('*Large MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_large) / max(1., count_large),
                                                      mse=math.sqrt(scalar_float(mse_large) / max(1., count_large))))

    print('*Cloudy MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_cloudy) / max(1., count_cloudy),
                                                       mse=math.sqrt(scalar_float(mse_cloudy) / max(1., count_cloudy))))
    print('*Sunny MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_sunny) / max(1., count_sunny),
                                                      mse=math.sqrt(scalar_float(mse_sunny) / max(1., count_sunny))))
    print('*Night MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_night) / max(1., count_night),
                                                      mse=math.sqrt(scalar_float(mse_night) / max(1., count_night))))

    print('*Crowded MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_crowd) / max(1., count_crowd),
                                                        mse=math.sqrt(scalar_float(mse_crowd) / max(1., count_crowd))))
    print('*Sparse MAE {mae:.1f} MSE {mse:.1f}'.format(mae=scalar_float(mae_sparse) / max(1., count_sparse),
                                                       mse=math.sqrt(scalar_float(mse_sparse) / max(1., count_sparse))))
