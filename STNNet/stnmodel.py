...
class STNNet(nn.Module):
    def __init__(self, use_loc=False, use_trk=False):
        ...
        mod = models.vgg16(pretrained = False)  # 不下载 ImageNet 权重，STNNet checkpoint 会覆盖
    ...
    def forward(self, imgset):
        ...
        for i in range(len(imgset)):
            ...
            for k in range(loc.shape[0]):
                ...
                n_sel = real_pts.shape[0]  # 实际点数, 可能 < topk

                # 极端情况: NMS 没有产生任何点
                if n_sel == 0:
                    empty_scores = outscs.unsqueeze(0)     # [1, 0]
                    empty_offsets = torch.zeros(1, 0, 2, device=pts.device)
                    empty_points = torch.zeros(1, 0, 2, device=pts.device, dtype=torch.long)
                    if k == 0:
                        random_scores = empty_scores
                        random_offsets = empty_offsets
                        random_points = empty_points
                    else:
                        random_scores = torch.cat((random_scores, empty_scores), 0)
                        random_offsets = torch.cat((random_offsets, empty_offsets), 0)
                        random_points = torch.cat((random_points, empty_points), 0)
                    continue
                ...
                denom = float(maxpoint - minpoint)
                if denom < 1e-8:
                    denom = 1.0
                pts_norm = (pts - minpoint) / denom

                # ★ 修复: 点数不足 topk 时 zero-pad, GCN 输出后再截断
                # GCN 内部 fixed npoint=128, 输入必须 >= 128 个点
                if n_sel < topk:
                    pad_pts = torch.zeros(topk - n_sel, 2, device=pts.device, dtype=pts.dtype)
                    pad_fts = torch.zeros(topk - n_sel, fts.shape[1], device=fts.device, dtype=fts.dtype)
                    pts_gcn = torch.cat([pts_norm, pad_pts], dim=0)
                    fts_gcn = torch.cat([fts, pad_fts], dim=0)
                else:
                    pts_gcn = pts_norm
                    fts_gcn = fts

                pts_gcn = pts_gcn.unsqueeze(0).transpose(2, 1)
                fts_gcn = fts_gcn.unsqueeze(0).transpose(2, 1)

                # extract locations and offsets based on gcns
                random_offset = self.gcn_layers(pts_gcn, fts_gcn)
                random_offset = random_offset.permute(0, 2, 1).contiguous()
                random_offset = random_offset[:, :n_sel, :]  # 截断回实际点数
