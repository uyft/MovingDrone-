...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 use_abs_pos_emb=False, interval=3, bn_group=None, test_pos_mode='simple_interpolate',
...
def vit_base_patch16(pretrained=False, load_pos_embed=True, **kwargs):
    ...
    #
    # # partial finetune
    # fix_backbone_partial_layers=\
    #     ['backbone_module.cls_token', 'backbone_module.pos_embed', 'backbone_module.patch_embed.proj.weight', 'backbone_module.patch_embed.proj.bias', '0', '1', '2', '3', '4']
    #
    # for name, params in model.named_parameters():
    #     print(name)
    #     if name in fix_backbone_partial_layers:
    #         params.requires_grad = False
    #
    #         print('fix param: {}'.format(name))
    #     else:
    #         curr_block_num = 'backbone_module.'+name
    #         if curr_block_num in fix_backbone_partial_layers:
    #             print('fix param: {}'.format(curr_block_num))
    # if pretrained:
...
def vit_large_patch16(pretrained=False, load_pos_embed=True, **kwargs):
    default = dict(
        drop_path_rate=0, use_abs_pos_emb=True,  # as in table 11
        ####
...
class MAEvitBackbone(object):
    def __init__(self, configer):
        ...
        import yaml
        f = open('./lib/models/backbones/maevit/config.yaml')
        cfg = f.read()
